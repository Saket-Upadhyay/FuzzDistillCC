...
LINUX_BITC_PATH = os.path.abspath(os.path.realpath("./Juliet1_3LinuxBitcs"))
MAC_BITC_PATH = os.path.abspath(os.path.realpath("./Juliet1_3MacBitcs"))
...
def clear_data(DATAPATH=TARGET_CSV_DIR):
    for csvs in tqdm([os.path.join(DATAPATH, cf) for cf in os.listdir(DATAPATH) if cf.endswith('.csv')],
        ...
        os.remove(csvs)  # print(f"Removed {csvs}")
